...
def stream_data(query, settings):
    ...
    try:
        ...
        logger.exception("Error to connect with the API")
        raise InterruptedError("No se pudo conectar con el API")
    except requests.RequestException as e:
        logger.exception(f"Request failed: {e}")
        raise InterruptedError(f"Error al conectar con el API {e}")
